...
"""
function trap_sim(sde_xo, ode_xo, tspan, feedback_params=(0.0,0.0,0.0), ode_abstol=1e-14, ode_reltol=1e-6, sde_abstol=1e-11, sde_reltol=1e-8)
    ...
    prob_sde = SDEProblem(sde_drift!, sde_diffusion!, sde_xo, tspan, p,
                           noise_rate_prototype = zeros(4, 3))
    sol_sde = solve(prob_sde, reltol=sde_abstol, abstol=sde_reltol, alg_hints=[:additive])
