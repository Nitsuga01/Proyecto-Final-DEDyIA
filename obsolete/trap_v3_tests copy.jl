...
sde_xo = [sqrt.(ode_xo[[1, 3, 8, 10]])...,0.0,0.0]
tspan = (0.0, 100.0)
ux=-0.05
uy=-0.05
vx=1
...
sol_sde, sol_ode = trap_sim(tspan, sde_xo, ode_xo, (ux,uy,vx,vy,errx,erry); sde_abstol=1e-7, sde_reltol=1e-5) 
...
time_series = 0.0:0.1:30.0
sol_ensamble, sol_ode = ensemble_trap_sim(1000,(time_series[1],time_series[end]))
summ = EnsembleSummary(sol_ensamble, time_series, quantiles=[0.05,0.95])
mean=summ.med[1:2,:]
q1=summ.qlow[1:2,:]
q2=summ.qhigh[1:2,:]

fig6 = Figure()
ax61 = Axis(fig6[1,1], xlabel="t (us)", ylabel= "Pos (nm)")
ax62 = Axis(fig6[1,2], xlabel="t (us)", ylabel= "Pos (nm)")
band!(ax61, time_series, q1[1,:], q2[1,:])
lines!(ax61, time_series, mean[1,:], label = "x")
band!(ax62, time_series, q1[2,:], q2[2,:])
lines!(ax62, time_series, mean[2,:], label = "y")
fig6