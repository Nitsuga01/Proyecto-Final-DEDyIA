...
#import DifferentialEquations: ODEProblem, SDEProblem, SDEFunction, SDDEProblem, SDDEFunction, solve
using DifferentialEquations, StochasticDiffEq, DelayDiffEq
...
function sde_drift!(du,u,h,p,t)
    # valor medio de posición y momento en la trampa
    x, y, px, py = u
    px_old = h(p,t-tau; idxs=3)
    py_old = h(p,t-tau; idxs=4)

    # fuerza de feedback y segundos momentos de posición y momento en la trampa´.
    ux, uy, err, C_xx, C_yy, C_xy, C_xpx, C_xpy, C_ypx, C_ypy = p
...
function sde_diffusion!(du,u,h,p,t)
    # fuerza de backaction y segundos momentos de posición y momento en la trampa
    ux, uy, err, C_xx, C_xy, C_yy, C_xpx, C_xpy, C_ypx, C_ypy = p
...
"""
function trap_sim(sde_xo, sde_h, ode_xo, tspan, feedback_params=(0.0,0.0,0.0,0.0), ode_abstol=1e-14, ode_reltol=1e-6, sde_abstol=1e-11, sde_reltol=1e-8)
    # estas son las constantes de proporcionalidad en x e y de la fuerza que se opone al momento
    # el delay con el que se mide el momento, y el error en la medición del mismo.
    ux, uy, tau, err = feedback_params
    ...
    # Luego simulo la SDE
    prob_sde = SDDEProblem(sde_drift!, sde_diffusion!, sde_xo, sde_h, tspan, p,
                           noise_rate_prototype = zeros(4, 3), constant_lags=(tau,))
    sol_sde = solve(prob_sde, ImplicitEM(), reltol=sde_abstol, abstol=sde_reltol)
