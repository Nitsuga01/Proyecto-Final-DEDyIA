...
sol_sde, sol_ode = trap_sim(initial_state_sde, initial_state_ode, tspan, (0.0,0.0), 1e-14, 1e-6, 1e-11, 1e-8) 
...
## probando el feedback

T = 2 # cada cuanto se toman mediciones.
err = 1e-3 # cuanto error gaussiano se agrega a las mediciones en um.
weight = 0.9 # indica cuanto peso debería darle a las nuevas mediciones al estimar el momento del sistema
fb_elasticity = 0.2/T # constante de proporcionalidad para la fuerza opuesta al momento.

trap_with_filter((0.0,100.0), [T, err, fb_elasticity, weight])
