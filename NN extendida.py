...
def generar_simulacion(Gamma_x, Gamma_y, Omega, tau_c, sigma_OU):
...
for n in range(Nsim):
    ...
    Gamma_x_sim = (0.2*np.random.randn() + 1) * Gamma_x

    Gamma_y_sim = (0.2*np.random.randn() + 1) * Gamma_y

    Omega_sim = (0.1*np.random.randn() + 1) * Omega

    tau_c_sim = (0.1*np.random.randn() + 1) * tau_c

    sigma_OU_sim = (0.1*np.random.randn() + 1) * sigma_OU

    curvas, x2_exp, y2_exp, px2_exp, py2_exp = generar_simulacion(Gamma_x_sim, Gamma_y_sim, Omega_sim, tau_c_sim, sigma_OU_sim)

    X.append(curvas)
    Y.append([Gamma_x_sim,Gamma_y_sim,Omega_sim, tau_c_sim, sigma_OU_sim])
...
curva, x2_exp, y2_exp, px2_exp, py2_exp = generar_simulacion(Gamma_x, Gamma_y, Omega, tau_c, sigma_OU)
...
print(Gamma_x, Gamma_y, Omega, tau_c, sigma_OU)
...
print(f"t_c = " f"{tauc_mean:.6f}" f" ± " f"{tauc_std:.6f}")

print(f"sigma = " f"{sigmaOU_mean:.6f}" f" ± " f"{sigmaOU_std:.6f}")
...
tauc_IC = np.percentile(tauc_boot, [2.5,97.5])

sigmaOU_IC = np.percentile(sigmaOU_boot, [2.5,97.5])
